...
def create_audio_tab(main_app, parent_tab):
    ...
    ttk.Label(config_frame, text="Tên Deck (để trống để quét tất cả):").pack(anchor="w")
    ...
    ttk.Label(fields_frame, text="Trường từ vựng:").grid(row=0, column=0, sticky="w", padx=(0, 10))
    ttk.Entry(fields_frame, textvariable=main_app.source_field_var, width=15).grid(row=0, column=1, sticky="w", padx=(0, 20))
    
    ttk.Label(fields_frame, text="Trường Audio:").grid(row=0, column=2, sticky="w", padx=(0, 10))
    ttk.Entry(fields_frame, textvariable=main_app.audio_field_var, width=15).grid(row=0, column=3, sticky="w")
    
    audio_settings_frame = ttk.LabelFrame(parent_tab, text="Cài đặt Audio", padding="10")
    ...
    ttk.Label(settings_row1, text="Tốc độ:").pack(side="left")
    ...
    ttk.Checkbutton(audio_settings_frame, text="Ghi đè audio đã có", variable=main_app.overwrite_audio_var).pack(anchor="w", pady=5)
